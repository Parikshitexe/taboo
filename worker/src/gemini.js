...
export async function generateRoast(roastContext, apiKey) {
    if (!apiKey) {
    ...
    const response = await fetch(`${GEMINI_URL}?key=${encodeURIComponent(apiKey)}`, {
        ...
        body: JSON.stringify({
            contents: [
                {
                    role: "user",
                    parts: [{ text: buildPrompt(roastContext) }]
                }
            ]
        })
    });
    ...
    if (!response.ok) {
        ...
        throw new Error(`Gemini HTTP ${response.status}: ${details.slice(0, 200)}`);
    }

    const data = await response.json();
    const roast = data?.candidates?.[0]?.content?.parts
        ?.map(part => part.text || "")
        .join("")
        .trim();
